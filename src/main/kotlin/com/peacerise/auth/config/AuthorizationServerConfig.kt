...
import com.peacerise.auth.portal.enhanced.JdbcRegisteredClientRepositoryEnhanced
...
import com.peacerise.auth.extend.password.OAuth2ResourceOwnerPasswordCredentialsAuthenticationConverter
import com.peacerise.auth.extend.password.OAuth2ResourceOwnerPasswordCredentialsAuthenticationProvider
import com.peacerise.auth.extend.proxybyapp.OAuth2ResourceAdminAppProxyAuthenticationConverter
import com.peacerise.auth.extend.proxybyapp.OAuth2ResourceAdminAppProxyAuthenticationProvider
import com.peacerise.auth.extend.proxybyuser.OAuth2ResourceAdminUserProxyAuthenticationConverter
import com.peacerise.auth.extend.proxybyuser.OAuth2ResourceAdminUserProxyAuthenticationProvider
...
class AuthorizationServerConfig {
    ...
    fun authorizationServerSecurityFilterChain(
        ...
        //Proxy login by app token testing
        //todo 如果新加一个不在AuthorizationGrantType中的,那么就不限制,否则需要在注册客户端时注册再允许!!! tbd 有时间可以看看这块代码
        http.getConfigurer(OAuth2AuthorizationServerConfigurer::class.java).tokenEndpoint {
            it.authenticationProvider(
                OAuth2ResourceAdminAppProxyAuthenticationProvider.Builder(
                    http,
                    userDetailService,
                    passwordEncoder
                ).build()
